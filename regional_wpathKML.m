...
%based on scaling the Earth to the scaled depth value.
%
%***!!!REQUIRES MatTaup to be installed!!!***
%
%Inputs: b_depth: scaled depth
%        evt: event location matrix nx3 containing n events' lat lon and
%             depth(km)
...
%phases and model
phase={'P'}; %multiple phases can be used
model='iasp91';
%output KML file
outfile='example_raypaths';

hexc=colormap2hex('jet',1); %colors by bottoming depth
...
for ect=1:numel(evt(:,1))
    ...
    for ct=1:numel(sta(:,1))
        ...
        for n=1:numel(TT)      %uncomment to show all arrivals
            X=[TT(n).path.longitude TT(n).path.latitude (b_depth-TT(n).path.depth).*1000.*scaling];
            cmp=round((max(TT(n).path.depth)/800)*64);

                fprintf(fid,'	<Placemark>\n');
                fprintf(fid,['		<name>' TT(n).phase '</name>\n']);
                fprintf(fid,'       <Style id="transPurpleLineGreenPoly">\n');
                fprintf(fid,'           <LineStyle>\n');
                fprintf(fid,['               <color>' hexc(cmp,:) '</color>\n']);
